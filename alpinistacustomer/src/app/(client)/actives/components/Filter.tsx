...
interface FilterProps {
    ...
    onApplyFilters: (filters: { issues: string; ip: string; port: string; severity: SeverityTypeEnum | null }) => void;
}

const Filter: React.FC<FilterProps> = ({ onSearch, onSeverityChange, onApplyFilters }) => {
    ...
    const handlePortInput = (value: string, setter: React.Dispatch<React.SetStateAction<string>>) => {
    ...
    const handleIssueOrRiskChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        <div>
            <section className="hidden lg:flex gap-3 w-full my-6 items-center">
                ...
                <div className="flex flex-col w-2/4 gap-2">
                    <label className="font-inter text-[12px] font-semibold  text-left" htmlFor="search">Pesquisar</label>
                                ...
                                {severityList.map((severity) => (
                                    <div key={severity.value} className="flex items-center gap-3">
                                        <Checkbox
                                            ...
                                            onCheckedChange={() => handleSeverityChange(severity.value)} // Use onCheckedChange
                                        />
                ...
                <div className="flex w-1/4 mt-8">
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        <DialogTrigger asChild>
                            <Button className="w-36 bg-[#3088EE]" onClick={() => setIsDialogOpen(true)}>
                                Filtrar
                            </Button>
                        </DialogTrigger>
                            ...
                            <div className="flex flex-col gap-6">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <Input
                                        type="text"
                                        value={portValue}
                                        onChange={(e) => handlePortInput(e.target.value, setPortValue)} // Permite apenas números
                                    ...
                                    <div>
                                        <Select onValueChange={handleDialogSeverityChange}>
                                            ...
                                            <SelectContent className="w-[240px]">
                                                {severityList.map((severity) => (
                                                    <SelectItem key={severity.value} value={severity.value.toString()}>
                                                        {severity.label}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                ...
                                <Button onClick={handleApplyFilters} className="w-full py-7 bg-[#3088EE] mt-4">Aplicar filtros</Button>
                            </div>
                        </DialogContent>
                    </Dialog>
                </div>
